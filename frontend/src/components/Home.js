  ...
  useEffect(() => {
    subscribeToMore({
      ...
      updateQuery: (prev, { subscriptionData }) => {
        if (!subscriptionData.data) return prev;
        var newPrev = prev;
        // console.log("prev:", prev.items);
        // console.log("subscriptionData:", subscriptionData);
        console.log(subscriptionData.data.itemDeleted);
        const deletedId = subscriptionData.data.itemDeleted;
        // const idx = prev.items.findIndex((obj) => obj.id === deletedId);
        // console.log(idx);
        // console.log(newPrev.items);
        // var arr = [1, 2, 3, 4, 5];
        // arr.splice(idx, 1);
        // console.log(arr);
        // if (idx > -1) {
        //   newPrev.items.splice(idx, 1);
        // }
        console.log(newPrev.items);
        return {
          items: prev.items.filter((obj) => obj.id !== deletedId),
        };
      },
    });
  }, [subscribeToMore]);
